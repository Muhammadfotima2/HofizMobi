...
def format_body(customer, phone, comment, total, currency) -> str:
...
def send_order():
    ...
    order_id = str(p.get("orderId", "N/A"))
    customer = str(p.get("customerName", "Клиент"))
...
def send_to_token():
    ...
    p = request.get_json(force=True, silent=True) or {}
    token = p.get("token")
    title = str(p.get("title", "Тест"))
    body  = str(p.get("body", "Привет!"))
    if not token:
        return Response(json.dumps({"ok": False, "error": "no token"}, ensure_ascii=False),
                        status=400, content_type="application/json; charset=utf-8")
    msg = messaging.Message(
